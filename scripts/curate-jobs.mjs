...
const INCLUSIVITY_SIGNALS_MAP = [
  { badge: 'Salary Transparent', regex: /([$£€]\s*\d{1,3}(?:,\d{3})*(?:k|\b)|compensation band|salary range|transparent pay)/i },
  ...
  { badge: 'Learning Stipend', regex: /(learning budget|education stipend|conference budget|development stipend|tuition reimbursement|\$1,500[^\n\r]{0,80}education|\$2,000[^\n\r]{0,80}learning)/i },
...
// ── Vetting Evaluation Helpers ───────────────────────────────────────────────
function validateJobSalary(job, highlights, issues) {
  const hasSalary = Boolean(
    ((job.salaryRange?.trim().length ?? 0) > 3 && !job.salaryRange.includes("Competitive & Transparent Band")) ||
    ((job.minSalary ?? 0) > 0)
  );
  if (!hasSalary) {
...
function validateJobWorkplace(job, issues) {
  const validModels = ['Remote (Global)', 'Remote (US/Americas)', 'Remote (UK/Europe)', 'Hybrid', 'On-site'];
  if (!job.remoteStatus || !validModels.includes(job.remoteStatus)) {
    issues.push("AMBIGUOUS_LOCATION: Workplace model (Remote/Hybrid/On-site) not clearly specified.");
  }
}

function validateInclusiveLanguage(fullText, issues) {
...
function detectInclusivityBadges(fullText, highlights) {
...
export function evaluateJobListing(job) {
  ...
  validateJobSalary(job, highlights, issues);
  validateJobWorkplace(job, issues);
  validateInclusiveLanguage(fullText, issues);
  detectInclusivityBadges(fullText, highlights);
  ...
  const fallbackSlug = `${companySlug}-${titleSlug}`.replace(/-+/g, '-').slice(0, 90);
...
function parseCliArgs(argv) {
  ...
  return { isDryRun, syncSanity, outputPath };
}

function scanAllFeeds(feeds) {
  const results = [];
  let passedCount = 0;
  let rejectedCount = 0;
  ...
  return { results, passedCount, rejectedCount };
}

function exportJobsToJson(cleanedResults, outputPath) {
...
async function syncJobsToSanity(results) {
...
// ── CLI Main Runner ──────────────────────────────────────────────────────────
async function run() {
  const { isDryRun, syncSanity, outputPath } = parseCliArgs(process.argv);

  console.log("==================================================================");
  console.log("🌸 Unlock Her Tech — Inclusive Job Board Curation & Vetting CLI 🌸");
  console.log("==================================================================\n");

  const { results, passedCount, rejectedCount } = scanAllFeeds(CURATED_FEEDS);
  ...
  exportJobsToJson(cleanedResults, outputPath);

  if (syncSanity) {
    await syncJobsToSanity(results);
  }
...
if (import.meta.url === `file://${scriptArg.replaceAll('\\', '/')}` || scriptArg.endsWith('curate-jobs.mjs')) {
  try {
    await run();
  } catch (err) {
