...
interface HeroSectionProps {
  ...
  nowPlayingContent: React.ReactNode;
}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 lg:py-24">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="order-2 lg:order-1">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full mb-6 bg-brand-pink text-neutral-900">
          ...
          <div className="order-1 lg:order-2 relative pb-10 lg:pb-0">
            <div className="relative rounded-3xl overflow-hidden shadow-2xl aspect-4/5 max-h-135 bg-gray-200">
              ...
              <div className="absolute inset-0 bg-linear-to-t from-brand-coral/80 via-brand-coral/30 to-transparent" />
            ...
            <div className="absolute -bottom-2 right-0 lg:-right-6 rounded-2xl p-5 shadow-2xl w-60 bg-neutral-900">
              {loading || !LATEST ? (
                <div className="space-y-3">
                  ...
                  <div className="flex items-end gap-0.5 h-8">
                    {WAVE_HEIGHTS.map((h, i) => (
                      <div key={`wave-loading-${i}`} className="flex-1 rounded-full bg-white/10" style={{ height: `${h}%` }} />
                    ))}
                  </div>
                ...
                <>
                  ...
                  <div className="mt-2 relative h-1.5 w-full bg-gray-700 rounded-full overflow-hidden">
                    <div
                      className="absolute top-0 left-0 h-full rounded-full transition-all bg-brand-coral pointer-events-none"
                    ...
                    <input
                      ...
                      aria-label="Seek"
                      title={latestIsActive ? "Drag to seek" : "Press play first"}
                    />
                  </div>
                  <div className="flex justify-between text-gray-500 text-xs mt-1">
                    <span>{latestIsActive ? formatTime(currentTime) : "0:00"}</span>
                    <span>{latestIsActive && duration ? formatTime(duration) : LATEST.duration}</span>
                  </div>
                </>
              )}
            </div>
            ...
            {/* Episode badge */}
            <div className="absolute -top-4 -left-4 rounded-2xl px-4 py-3 shadow-lg flex items-center gap-2 bg-brand-yellow">
...
function LatestEpisodeSection({
  ...
  descExpanded,
  setDescExpanded,
}: Readonly<LatestEpisodeSectionProps>) {
  return (
...
function BlogSection({ featuredPosts }: Readonly<{ featuredPosts: BlogPost[] }>) {
...
export function HomePage() {
  ...
  if (loading) {
    nowPlayingContent = <div className="h-5 w-48 rounded-full bg-white/20 animate-pulse" />;
  } else if (LATEST) {
    nowPlayingContent = (
      <div className="text-[1.15rem] font-extrabold">
